...
- **M5 (hardware demo) — confirmed.** `demo/` planted exactly one invalid
  register value (`IO_BANK0.GPIO25_CTRL.FUNCSEL=0`, not a valid SIO
  function-select for GPIO25), `silicon` flagged it statically, and
  flashing the buggy firmware onto a real Pico confirmed the predicted
  failure: the LED stays off because GPIO25 is never routed to SIO. The
  labeled benchmark + precision/recall measurement is on hold pending
  firmware generated by independent AI tools (not just Claude), to keep
  the precision number honest.
...
demo/               M5: hardware demo (planted bug + predicted real-board misbehavior)
```
