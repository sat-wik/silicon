...
- **M5 — in progress.** `demo/` has a hardware demo ready (a one-register-
  value planted bug — invalid GPIO function-select — predicted to keep the
  onboard LED off on a real Pico; not yet run on physical hardware). The
  labeled benchmark + precision/recall measurement is intentionally on hold
  until the corpus includes firmware generated by independent AI tools
  (not just Claude), to keep the precision number honest rather than
  self-graded.
